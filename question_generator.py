...
## 核心要求
1. **题型由你自行决定**：分析文档内容后，自行判断每道题适合出成什么类型。可以是选择题、填空题、判断题、简答题、配对题、排序题、连线题……只要你觉得合理，什么题型都可以。在每道题的 `type` 字段中写明题型名称。
...
## 各题型字段说明
- **选择题**：必须有 `options` 对象（键为 A/B/C/D），`answer` **必须严格是单个字母**（"A"、"B"、"C"、"D" 之一），绝对不能包含选项文字或其他内容。只写字母。
